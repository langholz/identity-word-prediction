...
def parse_arguments():
    ...
    # Common
    parser.add_argument("--model_type", type = str, default = "GRU", help = "The model type to use: LSTM or GRU")
    parser.add_argument("--corpus_file_path", type = str, default = "ptb_char_corpus.pkl", help = "The file path to save/load the corpus constructed from the data")
    ...
    parser.add_argument("--embedding_size", type = int, default = 200, help = "The embedding size")
    parser.add_argument("--hidden_size", type = int, default = 200, help = "Amount of hidden units per layer")
    parser.add_argument("--dropout", type = float, default = 0.2, help = "The dropout applied to each layer")
    ...
    parser.add_argument("--model_file_path", type = str, default = "ptb_char_model.pt", help = "File path used to save the model")
    parser.add_argument("--train_batch_size", type = int, default = 20, help = "The training batch size")
    parser.add_argument("--eval_batch_size", type = int, default = 10, help = "The evaluation batch_size")
    parser.add_argument("--sequence_length", type = int, default = 35, help = "Length of the sequence")
...
def setup_corpus(config):
    ...
    if (file_exists(config.corpus_file_path)):
        ...
        corpus = ptb_processor.process_corpus_as_characters(corpus)
        corpus.save(config.corpus_file_path)
        print("Saved corpus to '" + config.corpus_file_path + "'...")

    print(corpus.vocabulary.items())
    return corpus
...
def evaluate(config, corpus, rnn, criterion, data_set):
    ...
    hidden = rnn.create_hidden(batch_size = config.eval_batch_size)
...
def train_epoch(config, corpus, rnn, criterion, learning_rate):
    ...
    total_loss = 0
    term_count = corpus.vocabulary.count()
    hidden = rnn.create_hidden(batch_size = config.train_batch_size)
